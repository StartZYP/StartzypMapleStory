...
function start() {
    if (cm.isQuestCompleted(2013))
        cm.sendNext("这是你…多亏了你，我才得以完成许多工作。现在我做了很多东西。如果你需要什么，请告诉我。");
    else {
        if (cm.isQuestCompleted(2010))
            cm.sendNext("你看起来不够强壮，买不了我的药水……");
        else
            cm.sendOk("我的梦想是到处旅行，就像你一样。然而，我的父亲不允许我这样做，因为他认为这是非常危险的。不过，如果我向他证明我不是他认为的那种软弱的女孩，他也许会说是的……");
        cm.dispose();
    }
}
...
function action(mode, type, selection) {
    ...
    if (status == 0){
        ...
        cm.sendYesNo("你会购买 #r" + selection + "#k #b#t" + item[0] + "#(s)#k? #t" + item[0] + "# 成本 " + item[1] + " 金币 首先，总数是 #r" + (item[1] * selection) + "#k 金币.");
        amount = selection;
    } else if (status == 3) {
        if (cm.getMeso() < item[1] * amount)
            cm.sendNext("你有可能缺少金币吗?请检查，看看你是否有一个空间背包，如果你至少有 #r" + (item[1] * selectedItem) + "#k 金币 在身上.");
        else {
