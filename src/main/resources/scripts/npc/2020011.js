...
function action(mode, type, selection){
    ...
    if (actionx["Mental"]){
        if (status == 0)
            cm.sendNext("做得很好，你完成了测试的精神部分。你已经明智地回答了所有的问题。我必须说，我对你在那里展示的智慧水平印象深刻。在我们开始下一步之前，请先把项链递给我。");
        else if (status == 1)
            cm.sendYesNo("可以！现在，你将通过我变成一个更强大的冒险家。不过，在进行这项工作之前，请确保您的SP已被彻底使用，您至少需要在70级之前用完所有获得的SP才能获得第三个工作晋升。哦，既然你已经选择了第二职业副手的职业道路，你就不必再纠结第三职业。你现在想这样做吗？");
        else if (status == 2) {
            if (cm.getPlayer().getRemainingSp() > 0)
                if (cm.getPlayer().getRemainingSp() > (cm.getLevel() - 70) * 3) {
                    cm.sendNext("请在继续之前使用完你所有的SP.");
                    cm.dispose();
                    return;
                }
            if (cm.getJobId() % 10 == 0) {
            ...
            if(Math.floor(cm.getJobId() / 10) == 41) cm.sendNext("从现在起，你被正式任命为#bHermit#k . T技能书为隐士介绍了一系列新的攻击技能，使用阴影作为复制和替换的方式，包括 #bShadow Meso#k (用介子替换MP，根据投掷的介子数量用伤害来攻击怪物) 和 #bCopy cat#k (它模仿每一个动作，使隐士能够像两个隐士一样攻击怪物). 使用这些技能来对付以前可能难以征服的怪物。");
	    else cm.sendNext("从现在起，你被正式任命为 #bChief Bandit#k . 技能书的一个新添加是一种叫做 #bBand of Thieves#k, 在这个技能中，你可以召唤其他强盗同时攻击多个怪物。 强盗还可以通过多种方式利用媒介攻击怪物(#bMeso Explosion#k, 利用金币作为媒介在地面上爆炸), 来防卫自己 (#bMeso Guard#k, 减少武器伤害).");
