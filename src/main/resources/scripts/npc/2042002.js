        ...
        if (cm.getPlayer().getMapId() == 980000010) {
            ...
            if (status == 0) {
                ...
                if (selection == 0) {
                    ...
                    cm.sendNext("当你进入嘉年华领域，你会看到怪物列表窗口出现。您只需选择要使用的内容，然后按OK。非常简单，对吗?");
                } else {
                    cm.dispose();
                }
            } else if (status == 62) {
                if (select == 0) {
                    ...
                    cm.sendNext("一旦你习惯了这些命令,请尝试使用 #bF1~F12#k切换命令.#b选项卡在怪物调用之间切换技能#k,按#bF1~F12使你能够直接访问其中一个窗口#k.");
                }
            } else if (status == 63) {
                if (select == 0) {
                    cm.sendNext("我知道怪物嘉年华对你们来说太危险了，我不建议你们这么野蛮。不是我的朋友，是我为竞争对手提供的。战斗的兴奋和与如此强大和有动力的人竞争的兴奋。我提供的前提是你的团队和相反的团队 #b召唤怪物，击败对方召唤的怪物。这就是怪物嘉年华的精髓。此外，你可以使用在怪物狂欢节期间获得的硬币来获取新物品和武器！#k");
                } else if (select == 1) {
                    cm.sendNext("有三种方法可以分散对方的注意力:#b召唤怪兽,特殊技能和治疗药水#k.如果你想知道更多关于“详细说明”的信息,我会给你更深入的了解'!");
                } else if (select == 2) {
                    cm.sendNext("#b召唤#k一个怪物,在它的控制下攻击对方.它会出现攻击对方.");
                }
            } else if (status == 64) {
                if (select == 0) {
                ...
                cm.sendNext("最后，在怪物嘉年华期间，你不能使用随身携带的物品/恢复药剂。与此同时，怪物们让这些东西永远掉下来。当您得到它们时，该项目将立即激活。所以知道什么时候买这些东西很重要。");
                cm.dispose();
            } else if (status == 77) {
                var allDone;
                ...
                if(allDone) {
                    ...
                    cm.sendOk("完成了. 注意其中的一些项目 #r不能合成#k 因为要么你的ETC库存空间不足，要么没有足够的金币来支付费用。");
                }
                cm.dispose();
            }
        }
