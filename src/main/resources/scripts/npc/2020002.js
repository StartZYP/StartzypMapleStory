...
function action(mode, type, selection) {
    if (mode == 1)
        ...
        cm.dispose();
    if (status == 0 && mode == 1) {
        var selStr = "嗨,我是#b高登#k,有什么我可以帮助你的？？#b"
        var options = new Array("做战士鞋子", "做弓箭手鞋子", "做法师鞋子", "做盗贼鞋子");
        ...
        if (selectedType == 0) { //warrior shoes
            selStr = "需要什么吗？？#b";
            var shoes = new Array("#t1072147##k - 战士 等级. 60#b", "#t1072148##k - 战士 等级. 60#b", "#t1072149##k - 战士 等级. 60#b",
        ...
        } else if (selectedType == 1) { //弓箭手 shoes
            selStr = "需要什么吗？？#b";
        ...
        if (!complete)
            cm.sendOk("很抱歉由于你的材料不足，请确认后再联系我。");
        else {
            if (mats instanceof Array) {
                for (var i = 0; i < mats.length; i++) {
