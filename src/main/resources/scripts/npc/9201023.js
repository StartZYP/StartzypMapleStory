...
function processNanaQuest() {
    if(cm.haveItem(questItems[nanaLoc], 50)) {
        if(cm.canHold(4031367 + nanaLoc, 1)) {
            ...
            cm.sendOk("Kyaaaa~非常感谢你，给你 #b#t4031367##k.");
            return true;
        } else {
            cm.sendOk("请提供一个免费的其他栏空位，用来存放爱的信物.");
        }
    } else {
    ...
    if (mode == -1) {
        ...
        if(status == 0) {
            if(!cm.isQuestStarted(100400)) {
                cm.sendOk("你好 #b#h0##k, 我是 #p9201023# 爱的精灵.");     // thanks Periwinks (LuckyStory) for noticing Nana's introducing themselves as Nana (H)
                cm.dispose();
                return;
            }
            ...
            if(nanaLoc == -1) {
                cm.sendOk("你好 #b#h0##k, 我是 #p9201023# 爱的精灵.");
                cm.dispose();
                return;
            }
            ...
            if(!cm.haveItem(4031367 + nanaLoc, 1)) {
                if(cm.isQuestCompleted(100401 + nanaLoc)) {
                    state = 1;
                    cm.sendAcceptDecline("你把我给你的#k#t4031367##k卡丢了吗？好吧，我可以再和你分享一个，但你需要重做我上次问你的那件事，可以吗？我需要你带我来 #r50 #t" + questItems[nanaLoc] + "#'s.#k");
                    ...
                    state = 0;
                    cm.sendAcceptDecline("你在找#k#t4031367#'s#k吗？我可以和你分享一个，但你必须帮我一个忙，可以吗?");
                }
            } else {
                cm.sendOk("嘿，在那儿。你已经从另一家奶奶那里拿到 #t4031367#了吗？");
                cm.dispose();
            }
        } else if(status == 1) {
            if(state == 0) {
