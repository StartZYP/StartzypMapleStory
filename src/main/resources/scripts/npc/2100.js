...
function start() {
    if (cm.c.getPlayer().getMapId() == 0 || cm.c.getPlayer().getMapId() == 3)
        cm.sendYesNo("欢迎来到冒险岛.这个训练营的目的是帮助初学者。你想参加这个训练营吗？有些人没有参加训练计划就开始了他们的旅程。但我强烈建议你先参加训练计划.");
    else
        cm.sendNext("这是你的第一个训练计划开始的图像室。在这个房间里，你将预先调查你所选择的事情.");
}
...
function action(mode, type, selection) {
    ...
    if(status == 0)
        cm.sendPrev("一旦你足够努力地训练，你就有资格专职。你可以成为射手村的弓箭手，魔法密林的魔术师，勇士部落的勇士，废弃都市的小偷。。。");
    else
        cm.dispose();
}
