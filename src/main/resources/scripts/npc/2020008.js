...
function start() {
    if(cm.isQuestStarted(6192)) {
        ...
        if(eli.size() > 0) {
            ...
            cm.sendOk("您还无法开始，因为您的队伍不在同一张地图内，或者您的队员没有资格，或者他们不在此地图中。");
        }
...
function action(mode, type, selection){
    ...
    if (actionx["Mental"]){
        if (status == 0)
            ...
            cm.sendYesNo("可以！现在，你将通过我变成一个更强大的战士。不过，在这之前，请确保您的技能点已被彻底使用，您将需要使用至少所有技能点获得的，直到70级。哦，既然你已经选择了第二职业道路，你就不必再为第三职业选择了。你想现在就做吗？");
        else if (status == 2) {
