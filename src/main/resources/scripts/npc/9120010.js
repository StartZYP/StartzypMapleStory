...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            status++;
        if (status == 0) {
            cm.sendYesNo("如果你在找一个能精确指出各种物品特征的人，你现在就在找一个。我正在找东西。你想听我的故事吗");
        } else if (status == 1) {
            ...
            if (!cm.canHold(prizeItem)){
                cm.sendNext("如果你的道具栏满了，我不能给你奖励。请现在去看看。.");
            } else if (cm.hasItem(requiredItem, 100)){   // check they have >= 100 in Inventory
                cm.gainItem(requiredItem,-100);   
                cm.gainItem(prizeItem,prizeQuantity);
                cm.sendOk("嗯...如果不是为了这个小小的划痕....恐怕我只能将其视为标准品质的商品。这是你的 \r\n#t"+ 奖品 +"#.");
            } else{        
                cm.sendOk("嘿，你觉得你在干什么？去骗一个不知道他在说什么的人。不是我！");
            }
            cm.dispose();
        }
    }
