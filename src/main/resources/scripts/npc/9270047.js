        ...
        if (status == 0) {
            if (player.getLevel() < exped.getMinLevel() || player.getLevel() > exped.getMaxLevel()) { //Don't fit requirement, thanks Conrad
                ...
                if (expedition.contains(player)) { //If you're in it but it hasn't started, be patient
                    cm.sendOk("你已经登记参加远征队了。请等待#r" + expedition.getLeader().getName() + "#k to begin it.");
                    cm.dispose();
                ...
                if (expedition.contains(player)) { //If you're registered, warp you in
                    ...
                    cm.sendOk("另一支远征队主动挑战" + expedBoss + ", lets pray for those brave souls.");
                    cm.dispose();
                }
            }
        } else if (status == 1) {
            if (selection == 1) {
                if (!cm.haveItem(expedItem)) {
                    cm.sendOk("作为远征队的队长，你的背包中必须有#b#t" + expedItem + "##k才能战斗" + expedBoss + "!");
                    cm.dispose();
                    return;
                }
                ...
                expedition = cm.getExpedition(exped);
                if(expedition != null) {
                    cm.sendOk("有人已经主动成为远征队的领队。试着加入他们!");
                    cm.dispose();
                    return;
                }
                ...
                if (res == 0) {
                    ...
                    cm.sendOk("对不起，你已经达到这次进入的次数了！改天再试。。。");
                } else {
            ...
            if (selection == 1) {
                ...
                cm.sendOk("远征队将开始，你现在将被护送到#b" + expedMap + "#k.");
                status = 4;
            } else if (selection == 3) {
                ...
                cm.sendOk("远征队现在已经结束了。有时候最好的策略是逃跑。");
                cm.dispose();
                return;
            }
        } else if (status == 4) {
