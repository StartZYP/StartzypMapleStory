        ...
        if (status == 0) {
            if (player.getLevel() < exped.getMinLevel() || player.getLevel() > exped.getMaxLevel()) { //Don't fit requirement, thanks Conrad
                ...
                if (expedition.contains(player)) { //If you're in it but it hasn't started, be patient
                    cm.sendOk("你已经登记参加探险队了。请等待 #r" + expedition.getLeader().getName() + "#k 开始吧.");
                    cm.dispose();
                ...
                if (expedition.contains(player)) { //If you're registered, warp you in
                    ...
                    if(eim.getIntProperty("canJoin") == 1) {
                        ...
                        cm.sendOk("你的探险队已经开始与 " + expedBoss + ". 让我们为那些勇敢的灵魂祈祷。");
                    }
            ...
            if (selection == 1) {
                if (!cm.haveItem(expedItem)) {
                    cm.sendOk("作为探险队的领队，你的库存中必须有 #b#t" + expedItem + "##k 战斗 " + expedBoss + "!");
                    cm.dispose();
                    return;
                }
                ...
                if (res == 0) {
                    cm.sendOk("The #r" + expedBoss + " 远征队#k 已经创建.\r\n\r\n再和我谈谈，看看现在的队伍，或者开始战斗");
                } else if (res > 0) {
                    cm.sendOk("对不起，你已经达到这次探险的限额了！改天再试。。。");
                } else {
            ...
            if (selection == 1) {
                ...
                if (size < min) {
                    cm.sendOk("你至少需要 " + min + " 在您的探险队注册的玩家.");
                    cm.dispose();
                    return;
                }
            ...
            if(!em.startInstance(expedition)) {
                cm.sendOk("另一支探险队主动挑战 " + expedBoss + ", lets pray for those brave souls.");
                cm.dispose();
                return;
            }
            ...
            if (selection > 0) {
                ...
                cm.sendOk("你已经禁止了" + banned.getValue() + " from the expedition.");
                cm.dispose();
            } else {
                cm.sendSimple(list);
                status = 2;
