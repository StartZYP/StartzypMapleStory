...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0 && mode == 0) {
            ...
            cm.sendNext("以下是如何击败一个怪物的方法。每个怪物都拥有自己的HP，你会取下来用任何武器或通过法术攻击。成为然，它们越强大，就越难击败它们.");
        }
        if (mode == 1) status++;
        else status--;
        if (status == 0) {
            ...
            if (selection == 0) { // How do I move?
            ...
            } else if (selection == 7) { // What should I do to become a 弓箭手?
                status = -1;
                ...
                status = -1;
                cm.sendNext("在南港乘船去往金银岛。按下W键你可以确认你现在的位置。但要乘船需要一定的金币，请在这里打猎怪物赚钱吧。");
