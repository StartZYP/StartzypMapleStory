...
function start() {
    ...
    if(cm.getMapId() == 680000200) {
        ...
        if(cm.getPlayer().getId() != eim.getIntProperty("groomId") && cm.getPlayer().getId() != eim.getIntProperty("brideId")) {
            cm.sendNext("对不起，现在应该只有这对结婚的夫妇在和我说话。");
            cm.dispose();
            return;
        }
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (hasGoldenLeaf && hasEngage) {
            cm.sendOk("你还不能走！在我让你离开之前，你需要点击Pelvis Bebop并得到他的承诺。");
            cm.dispose();
            ...
            cm.sendNext("你好像没有订婚戒指或结婚戒指。你一定不属于这里，所以我带你去婚礼村");
            selection = 20; // Random.
        }
    } else if (status == 1) {
        var cmPartner;
        ...
        switch(selection) {
            ...
            case 1:
                cm.sendOk("超级巨星们必须得到佩尔维斯·贝博普的消息才能团结起来. 当你准备好了，你可以点击我去参加后派对。");
                cm.dispose();
                break;
