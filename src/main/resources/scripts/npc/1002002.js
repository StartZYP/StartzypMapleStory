...
function action(mode, type, selection) {
    ...
    if(status == 1){
        if(selection == 1)
            ...
            cm.sendNext("你一定对#b#t4031134##k很好奇。哈哈，只要你拥有你就可以免费前往黄金海滩。这是一件很少见的东西，连我们都不得不买，但不幸的是，几周前我弄丢了。");
    } else if (status == 2){
        if(type != 1 && selection != 0) {
            ...
            if (cm.getMeso() < 10000 && selection == 0)
                cm.sendNext("你的钱不够。有很多方法可以筹集到一些钱，你知道，比如。。。卖你的装备。。。打败怪物。。。执行任务。。。你知道我在说什么.");
