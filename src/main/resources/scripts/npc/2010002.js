...
function action(mode, type, selection) {
    if (mode < 1) {  // disposing issue with stylishs found thanks to Vcoc
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendSimple("你好,欢迎来到天空之城整形手术馆,你你想更换你的脸型吗? 有了#b#t5152005##k,你就可以把你的脸型更换成你想要的样子!\r\n#L2#使用#b#i5152005##t5152005##l");
        } else if (status == 1) {
            if (selection == 2) {
                facenew = Array();
                ...
                cm.sendStyle("我完全可以把你的脸变成新的。。。我们试试怎么样? 使用#b#t5152005##k,你可以得到你喜欢的面孔……花点时间选择你喜欢的面孔.", facenew);
            }
        }
        else if (status == 2){
            cm.dispose();
