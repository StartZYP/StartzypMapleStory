...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        if (cm.haveItem(4031801))
            cm.sendNext("好，现在给我#b150金币#k……嗯，那是什么？是路卡斯的#b推荐信#k？嘿，你应该告诉我。伟大的冒险家，我似乎已经看到你的将来！");
        else
            cm.sendNext("确定要离开吗？那么……先付我#b150金币#k吧……");
    } else if (status == 2) {
        if (cm.haveItem(4031801))
            cm.sendNextPrev("既然你有#b推荐信#k，我也不会向你收取任何费用的。好了，我们现在就向#b明珠港#k出发。船可能会有点动荡，坐好了……");
        else
        if (cm.getLevel() > 6) {
            if (cm.getMeso() < 150) {
                cm.sendOk("什么？你想去#b明珠港#k但你没有钱？你是一个怪人……");
                cm.dispose();
            } else
