...
public class MakerProcessor {
            ...
            try {
                ...
                if(type == 3) {    // building monster crystal
                    ...
                    if(ItemConstants.isEquipment(toCreate)) {   // only equips uses stimulant and reagents
                        if(slea.readByte() != 0) {  //  stimulant//lynfan 锻造面板里添加了促进剂时
                        ...
                        for(int i = 0; i < reagents; i++) {  // crystals//lynfan锻造面板里添加了强化宝石时
                ...
                switch(createStatus) {
                    ...
                    default:
                        ...
                        /*lynfan 原始代码
                        if (type == 3) {//lynfan 合成怪物结晶在会话栏出现的“获得物品XXXX 多少个，丢弃物品XXXX 多少个”
                        ...
                        */
                        ...
                        if (type == 3) {//lynfan 合成怪物结晶在会话栏出现的“获得物品XXXX 多少个，丢弃物品XXXX 多少个”
                            c.announce(MaplePacketCreator.makerResultCrystal(recipe.getGainItems().get(0).getLeft(), recipe.getReqItems().get(0).getLeft()));
                            final int itemlevel=ii.getlvMax(recipe.getGainItems().get(0).getLeft());//lynfan add， 判断分解出的怪物结晶等级
                            int expAward = (int)(itemlevel* Math.random()+1)* itemlevel * c.getPlayer().getExpRate() * c.getPlayer().getLevel()/5; //lynfan add分解结晶获得经验
                            c.getPlayer().gainExp(expAward, true, true);//lynfan add分解获得经验
                        } else if (type == 4) {
                            c.announce(MaplePacketCreator.makerResultDesynth(recipe.getReqItems().get(0).getLeft(), recipe.getCost(), recipe.getGainItems()));
                            final int equiplevel=ii.getReqLevel(recipe.getReqItems().get(0).getLeft());//lynfan add， 分解的装备等级
                            int expAward = (int)(equiplevel* (Math.random()+1) * equiplevel) * c.getPlayer().getExpRate() * c.getPlayer().getLevel()/5; //lynfan add分解装备获得经验
                            c.getPlayer().gainExp(expAward, true, true);//lynfan add分解装备获得经验
                            c.getPlayer().increaseEquipExp((int)(equiplevel* (Math.random()+1) * equiplevel) *c.getPlayer().getExpRate() * c.getPlayer().getLevel()/10);//lynfan add分解装备获得装备经验
                        } else {
                            c.announce(MaplePacketCreator.makerResult(makerSucceeded, recipe.getGainItems().get(0).getLeft(), recipe.getGainItems().get(0).getRight(), recipe.getCost(), recipe.getReqItems(), stimulantid, new LinkedList<>(reagentids.keySet())));
                        }
                        //above lynfan added
    ...
    private static Pair<Integer, List<Pair<Integer, Integer>>> generateDisassemblyInfo(int itemId) {
        int recvFee = ii.getMakerDisassembledFee(itemId);//lynfan备注，分解装备费用引用
        if(recvFee > -1) {
    ...
    private static boolean addBoostedMakerItem(MapleClient c, int itemid, int stimulantid, Map<Integer, Short> reagentids) {
        ...
        if(!reagentids.isEmpty()) {//lynfan如果合成装备时放置了强化宝石。
            ...
            ii.improveEquipStats(eqp, stats);//lynfan 强化宝石加成的属性来自improveEquipStats
        ...
        */
        ...
        //lynfan added
        if(stimulantid != -1) {//lynfan如果合成装备时放置了促进剂。
            //eqp = ii.randomizeUpgradeStats(eqp);//lynfan 原始代码
            final int equiplevel=ii.getReqLevel(itemid);//lynfan  促进剂加成的属性来着randomizeUpgradeStats1，lynfan修改的随机属性。
            eqp = ii.randomizeUpgradeStats1(eqp,equiplevel);// 促进剂加成的属性来着randomizeUpgradeStats1，lynfan修改的随机属性。
        }
        //lynfan added
