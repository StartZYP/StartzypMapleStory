...
public final class EnterMTSHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        NPCScriptManager.getInstance().dispose(c);//lynfan 去掉拍卖功能，开启拍卖npc脚本1101008
        NPCScriptManager.getInstance().start(c, 1101008, null);//lynfan 去掉拍卖功能，开启拍卖npc脚本1101008
        ...
        if(!chr.isAlive() && YamlConfig.config.server.USE_BUYBACK_SYSTEM) {
            ...
            if(chr.getEventInstance() != null) {
                c.announce(MaplePacketCreator.serverNotice(5, "不可以在活动中进入拍卖.Entering Cash Shop or MTS are disabled when registered on an event."));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            
            if(MapleMiniDungeonInfo.isDungeonMap(chr.getMapId())) {
                c.announce(MaplePacketCreator.serverNotice(5, "当你在特殊地图中是不允许进入拍卖的.Changing channels or entering Cash Shop or MTS are disabled when inside a Mini-Dungeon."));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
