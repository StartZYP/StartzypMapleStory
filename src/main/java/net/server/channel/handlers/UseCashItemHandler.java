...
public final class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (timeNow - player.getLastUsedCashItem() < 3000) {
            player.dropMessage(1, "你最近已经使用了商城物品，等一等后再重试.You have used a cash item recently. Wait a moment, then try again.");
        ...
        if (itemType == 504) { // vip teleport rock
            String error1 = "找不到玩家，或者您试图传送到非法位置.Either the player could not be found or you were trying to teleport to an illegal location.";
            ...
            if (!vip) {
                ...
                if (itemId / 1000 >= 5041 || mapId / 100000000 == player.getMapId() / 100000000) { //check vip or same continent
                    ...
                    player.dropMessage(1, "你不能用这个在大陆间传送。You cannot teleport between continents with this teleport rock.");
                }
            } else {
                ...
                if (victim != null) {
                    ...
                    if (!FieldLimit.CANNOTVIPROCK.check(targetMap.getFieldLimit()) && (targetMap.getForcedReturnId() == 999999999 || targetMap.getId() < 100000000)) {
                        ...
                        player.dropMessage(1, "你不能传送到这张地图。You cannot teleport to this map.");
                    }
                } else {
                    player.dropMessage(1, "频道中找不到这个玩家。Player could not be found in this channel.");
                }
            }
            ...
            switch ((itemId / 1000) % 10) {
                case 1: // Megaphone
                    if (player.getLevel() > 9) {
                        ...
                        player.dropMessage(1, "你需要等到10级后才可以使用.You may not use this until you're level 10.");
                        return;
                    }
                    break;
                ...
                case 5: // Maple TV
                    ...
                    if (!MapleTVEffect.broadcastMapleTVIfNotActive(player, victim, messages, tvType)) {
                        player.dropMessage(1, "冒险岛TV正在使用中MapleTV is already in use.");
                        return;
                    }
                ...
                case 7: //triple megaphone
                    ...
                    for (int i = 0; i < lines; i++) {
                        msg2[i] = medal + player.getName() + " : " + slea.readMapleAsciiString();//lynfan 原始代码
                    /* lynfan 缤纷喇叭 中文修复
                    byte[] read = slea.read(slea.readShort());
                        String s = null;
                        try {
                            s = new String(read,"GBK");
                        } catch (UnsupportedEncodingException e) {
                            e.printStackTrace();
                        }
                        msg2[i] = medal + player.getName() + " : " + s;    
                        */    
            ...
            player.getMap().startMapEffect(ii.getMsg(itemId).replaceFirst("%s", player.getName()).replaceFirst("%s",s), itemId);
            */
            ...
            if (GameConstants.isFreeMarketRoom(player.getMapId())) {
                player.dropMessage(5, "你不能在这里用黑板。You cannot use the chalkboard here.");
            ...
            for (int i = 0; i < 4; i++) {
                strLines.add(slea.readMapleAsciiString());//lynfan 原始代码
            /*lynfan added喇叭支持中文
            String s = null;
                try {
                    s = new String(read,"GBK");
            ...
            */    
            ...
            if(itemId == 5432000 && !c.gainCharacterSlot()) {
                player.dropMessage(1, "你已经使用完所有的12个额外的角色位You have already used up all 12 extra character slots.");
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            if(createStatus == 0) {
                ...
                player.showHint("#b完成#k 通过卡片创造新角色!.#bSuccess#k on creation of the new character through the Maple Life card.");
                remove(c, position, itemId);
            } else {
            ...
            System.out.println("新商城物品NEW CASH ITEM: " + itemType + "\n" + slea.toString());
            c.announce(MaplePacketCreator.enableActions());
        }
    }
