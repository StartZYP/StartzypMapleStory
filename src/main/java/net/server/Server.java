...
public class Server {
    ...
    private AutoLootTask AutoLootTask;
    ...
    private void initializeTimelyTasks() {
        ...
        tMan.register(new AutoLootTask(), 10 * 1000, 1000);//lynfan add 有宠物后自动拾取，1秒钟拾取一次，一个是第一次运行的等待时间  一个是两次之间的间隔时间
        tMan.register(new MapSleepGiftTask(),30*1000,1000);
...
//lynfan add，怪物偷袭
        ...
        if (YamlConfig.config.server.启用怪物偷袭) {
			...
			System.out.println(timeLeft / 1000 / 60 + "分钟后怪物开始偷袭");
