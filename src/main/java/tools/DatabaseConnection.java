...
public class DatabaseConnection {
    ...
    public static Connection getConnection() throws SQLException {
        ...
        while(true) {   // There is no way it can pass with a null out of here?
            try {
                //return DriverManager.getConnection(YamlConfig.config.server.DB_URL, YamlConfig.config.server.DB_USER, YamlConfig.config.server.DB_PASS);//lynfan 原始代码
                //lynfan added设置数据库字符编码啊，汉化，中文
                String url = YamlConfig.config.server.DB_URL + "?user=" + YamlConfig.config.server.DB_USER
						+ "&password=" + YamlConfig.config.server.DB_PASS + "&characterEncoding=utf8";
				return DriverManager.getConnection(url);
                //lynfan added  设置数据库字符编码啊，汉化，中文
