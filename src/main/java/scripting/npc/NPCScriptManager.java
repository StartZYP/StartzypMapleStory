...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    //lynfan added below
    	public boolean start(MapleClient c, int npc, String fileName, String dot, boolean extend) {
		try {
			NPCConversationManager cm = new NPCConversationManager(c, npc, -1, fileName, false);
			...
			if (c.canClickNPC()) {
				...
				NashornScriptEngine iv = null;
				if (fileName != null) {
					iv = getScriptEngine("npc/" + npc + dot + fileName + ".js");
				}
				if (iv == null) {
				...
				try {
					...
					try {
						iv.invokeFunction("start", c.getPlayer());
