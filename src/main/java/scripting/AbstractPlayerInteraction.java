...
public class AbstractPlayerInteraction {
	...
	public void openNpc(int npcid, String script) {
                ...
                /*if (c.getCM() != null) {
                    return;
                }*/ //原始代码lynfan remove 修复脚本里的cm.openNPC功能
        ...
        public void openNpc(int npcid, String script, String dot) {//lynfan added
		if (c.getCM() != null) {
			c.removeClickedNPC();
		}
		NPCScriptManager.getInstance().dispose(c);
		NPCScriptManager.getInstance().start(c, npcid, script, dot, true);
	}//lynfan added
	...
	public MapleExpedition getExpedition(MapleExpeditionType type) {
        ...
        public void removeItem(short slot,byte type,short quantity) {
		MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.getByType(type), slot, quantity, false,
				false);
	}
	public Equip getEquipInSlot(short slot) {
		return (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(slot);
	}
	public void eatEquip(MapleCharacter c,Equip oldEquip,Equip newEquip) {
		Equip.eatEquip(c, oldEquip, newEquip);
	}
         //lynfan 以上语句增加装备升级经验转移到新装备的功能
        
         //以下lynfan added, 增加cm.removeSlot(inventoryType, deleteSlot, deleteQuantity);分解装备
        ...
        //以上lynfan added, 增加cm.removeSlot(inventoryType, deleteSlot, deleteQuantity);分解装备
}