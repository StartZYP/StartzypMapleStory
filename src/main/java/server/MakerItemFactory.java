...
public class MakerItemFactory {
    ...
    public static MakerItemCreateEntry generateLeftoverCrystalEntry(int fromLeftoverid, int crystalId) {
        ...
        */
        MakerItemCreateEntry ret = new MakerItemCreateEntry(0, 0, 1);
        ret.addReqItem(fromLeftoverid, 100);//lynfan备注 怪物战利品合成怪物结晶，100为消耗的战利品数量
        final int qty = (int)(Math.ceil(Math.random() * 4));//lynfan Math.ceil() 向上取整, 这里返回的是1~5的整数。 
        ret.addGainItem(crystalId, qty);//lynfan备注 怪物战利品合成怪物结晶，1为合成的结晶数量

        System.out.println("合成了: " + qty + "怪物结晶.");        
//player.dropMessage("合成了 "+ qty +"怪物结晶");
        return ret;
    }
