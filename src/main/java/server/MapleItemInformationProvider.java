...
public class MapleItemInformationProvider {
    ...
    protected Map<Integer, Integer> lvMinCache = new HashMap<>();
    protected Map<Integer, Integer> lvMaxCache = new HashMap<>();
    ...
    //lynfan 增加getlvMax, 判断etc物品的最大等级
    public int getlvMax(int itemId) {
        if (lvMaxCache.containsKey(itemId)) {
            return lvMaxCache.get(itemId);
        }
        ...
        if (item != null) {
            ...
            if (info != null) {
                ret = MapleDataTool.getInt("lvMax", info, 0);
            }
        }
        
        lvMaxCache.put(itemId, ret);
        return ret;
    }
    
    public int getlvMin(int itemId) {
        if (lvMinCache.containsKey(itemId)) {
            return lvMinCache.get(itemId);
        }
        ...
        if (item != null) {
            MapleData info = item.getChildByPath("info");
            if (info != null) {
                ret = MapleDataTool.getInt("lvMin", info, 0);
            }
        }
        
        lvMinCache.put(itemId, ret);
        return ret;
    ...
    //below lynfan added,//判断装备等级
        ...
        public final int getReqJob(final int itemId) {//判断装备要求职业
    ...
    public Map<String, Integer> getEquipStats(int itemId) {
        ...
        //lynfan原始代码for (MapleData data : info.getChildren()) {
        //lynfan原始代码    if (data.getName().startsWith("inc")) {
        //lynfan原始代码        ret.put(data.getName().substring(3), MapleDataTool.getIntConvert(data));
        //lynfan原始代码    }
            //*else if (data.getName().startsWith("req"))
            // ret.put(data.getName(), MapleDataTool.getInt(data.getName(), info, 0));*/
        //lynfan原始代码}
        
        boolean isScroll = itemId / 10000 == 204;//lynfan 判断卷轴
        int delta = isScroll ? YamlConfig.config.server.SCROLL_STAT_RATE : 1;//lynfan 卷轴倍率
        for (MapleData data : info.getChildren()) {
            if (data.getName().startsWith("inc")) {
                ret.put(data.getName().substring(3), MapleDataTool.getIntConvert(data) * delta);//lynfan 增加delta，卷轴倍率
            }
            /*else if (data.getName().startsWith("req"))
             ret.put(data.getName(), MapleDataTool.getInt(data.getName(), info, 0));*/
        }
    ...
    */
    ...
    public static void improveEquipStats(Equip nEquip, Map<String, Integer> stats) {//lynfan 强化宝石加成的属性
...
//above lynfan原始掉落装备属性的代码    
    
//below lynfan 原始代码锻造装备属性加成算法
...
//above lynfan 原始代码锻造装备属性加成算法
    ...
    public List<Pair<Integer, Integer>> getMakerDisassembledItems(Integer itemId) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            //PreparedStatement ps = con.prepareStatement("SELECT req_item, count FROM makerrecipedata WHERE itemid = ? AND req_item >= 4260000 AND req_item < 4270000");//原始代码lynfan 这里req_item 限制了分解装备只得到怪物结晶
            PreparedStatement ps = con.prepareStatement("SELECT req_item, count FROM makerrecipedata WHERE itemid = ?");//lynfan 这里去掉req_item 限制。
            ps.setInt(1, itemId);
            ResultSet rs = ps.executeQuery();

            while (rs.next()) {
                items.add(new Pair<>(rs.getInt("req_item"), rs.getInt("count") / 2));   // return to the player half of the crystals needed//lynfan备注，分解装备的到的怪物结晶数量为数据库makerrecipedata 里 的count/2
            }
    ...
    public int getMakerDisassembledFee(Integer itemId) {//lynfan备注，分解装备费用
        int fee = -1;
        Connection con;
        try {
            con = DatabaseConnection.getConnection();
    ...
    //lynfan 锻造装备属性加成算法    
        private final short getRandUpgradedStat(final short equiplevel) {//lynfan , 力量，敏捷，运气和智力随机属性。
        if (Randomizer.nextInt(10) == 1) { //1/10的概率,该方法的作用是生成一个随机的int值，该值介于[0,n)的区间，也就是0到n之间的随机int值，包含0而不包含n
                return (short) ((Math.floor(Math.random() * (equiplevel/2+1)))+Math.floor(equiplevel/3));//lynfan 这里的equiplevel是指装备等级，根据装备等级设置随机属性:10%的概率出现随机属性为装备等级/3~装备等级/2之间的值的，比如装备等级30，那么随机属性就是10-15
            } 
            else 
            {
                return (short) (Math.floor(Math.random() * (equiplevel/3+1)));//lynfan Math.floor() 向下取整, 90%概率出现随机属性是0~装备等级/3之间的值，比如装备等级30，那么随机属性就是0-10
            }

        
        }
    private final short getRandUpgradedStat1(final short defaultValue, final int maxRange) {//lynfan added，区分物理攻击，魔法攻击，闪避，命中，跳跃，速度，物理防御，魔法防御，手技。
        if (defaultValue == 0) {
            return 0;
        }
        else {//如果锻造的装备属性项不为0，随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
            
            if (Randomizer.nextInt(10) == 1) { //1/10的概率
                return (short) (defaultValue + Math.floor(defaultValue * 0.4));//上限+40%
            } 
            else 
            {
                final int lMaxRange = (int) Math.min(Math.ceil(defaultValue * 0.1), maxRange);//lynfan Math.ceil() 向上取整，即大于这个数的最小的那个整数。
                return (short) ((defaultValue - lMaxRange) + Math.floor(Math.random() * (lMaxRange * 2 + 1)));//这里的2，上限+10%，下限都是-10%，如果改为4，上限就是+20%,下限还是-10%。
            }
            
        }
    }
    
     private final short getRandUpgradedStat2() {//lynfan added，区分HP，MP。
        //lynfan 如果锻造的装备其HP和MP属性项为0，随机属性的算法:10%的概率出现50~100的HP/MP值，剩余90%的概率出现0~50的HP/MP值
            if (Randomizer.nextInt(10) == 1) { //1/10的概率,该方法的作用是生成一个随机的int值，该值介于[0,n)的区间，也就是0到n之间的随机int值，包含0而不包含n
                return (short) (Math.floor(Math.random() * 501)+500);//lynfan Math.floor() 向下取整, 这里返回的是500~1000的整数。
            } 
            else 
            {
                return (short) (Math.floor(Math.random() * 51));//lynfan Math.floor() 向下取整, 这里返回的是0~50的整数。
            }

    }
    
      private final byte getRandUpgradedStat3() {//lynfan added，随机装备的可升级次数。10%概率在10~20之间，90%的概率在5~10之间。
        
            if (Randomizer.nextInt(10) == 1) { //1/10的概率,该方法的作用是生成一个随机的int值，该值介于[0,n)的区间，也就是0到n之间的随机int值，包含0而不包含n
                return (byte) (Math.floor(Math.random() * 11)+10);//lynfan Math.floor() 向下取整, 这里返回的是10~20的整数。
            } 
            else 
            {
               return (byte) (Math.floor(Math.random() * 6)+5);//lynfan Math.floor() 向下取整, 这里返回的是5~10的整数。 
            }

    }


    public Equip randomizeUpgradeStats1(final Equip equip,final int equiplevel) {//lynfan 锻造装备属性加成算法
        equip.setStr(getRandUpgradedStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setDex(getRandUpgradedStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setInt(getRandUpgradedStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setLuk(getRandUpgradedStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setMatk(getRandUpgradedStat1(equip.getMatk(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setWatk(getRandUpgradedStat1(equip.getWatk(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setAcc(getRandUpgradedStat1(equip.getAcc(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setAvoid(getRandUpgradedStat1(equip.getAvoid(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setJump(getRandUpgradedStat1(equip.getJump(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setWdef(getRandUpgradedStat1(equip.getWdef(), 50));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setMdef(getRandUpgradedStat1(equip.getMdef(), 50));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setHp(getRandUpgradedStat2());//lynfan 随机MP属性的算法:10%的概率出现500~1000的HP/MP值，剩余90%的概率出现0~50的HP/MP值
        equip.setMp(getRandUpgradedStat2());//lynfan 随机MP属性的算法:10%的概率出现500~1000的HP/MP值，剩余90%的概率出现0~50的HP/MP值
        equip.setUpgradeSlots(getRandUpgradedStat3());//lynfan,随机装备升级次数,10%概率在10~20之间，90%的概率在5~10之间。
        return equip;
    }
//lynfan 锻造装备属性加成算法  
    
//lynfan BOSS怪物掉落装备属性的代码 
    private final short getRandStat(final short equiplevel) {//lynfan , 力量，敏捷，运气和智力随机属性。
     if (Randomizer.nextInt(10) == 1) { //1/10的概率,该方法的作用是生成一个随机的int值，该值介于[0,n)的区间，也就是0到n之间的随机int值，包含0而不包含n
                return (short) ((Math.floor(Math.random() * (equiplevel/2+1)))+Math.floor(equiplevel/3));//lynfan 这里的equiplevel是指装备等级，根据装备等级设置随机属性:10%的概率出现随机属性为装备等级/3~装备等级/2之间的值的，比如装备等级30，那么随机属性就是10-15
            } 
            else 
    ...
    private final short getRandStat1(final short defaultValue, final int maxRange, final short equiplevel) {//lynfan added，区分物理攻击，魔法攻击。
        if (defaultValue == 0) {
            return (short) ((Math.floor(Math.random() * (equiplevel/5+1)))+Math.floor(equiplevel/10));//lynfan 这里的equiplevel是指装备等级，根据装备等级设置随机属性:10%的概率出现随机属性为装备等级/10~装备等级/5之间的值的，比如装备等级30，那么随机属性就是4-8
        
        }
        else {//如果装备属性项不为0，随机属性的算法：10%的概率出现属性直接加+50%，90%的概率出现上限+20%，下限都是-10%
            
            if (Randomizer.nextInt(10) == 1) { //1/10的概率
                return (short) (defaultValue + Math.floor(defaultValue * 1));//上限+100%
            } 
            else 
            {
                final int lMaxRange = (int) Math.min(Math.ceil(defaultValue * 0.1), maxRange);//lynfan Math.ceil() 向上取整，即大于这个数的最小的那个整数。
                return (short) ((defaultValue - lMaxRange) + Math.floor(Math.random() * (lMaxRange * 4 + 1)));//这里的2，上限+10%，下限都是-10%，如果改为4，上限就是+20%,下限还是-10%。
            }
            
        }
    }
        private final short getRandStat2(final short defaultValue, final int maxRange) {//lynfan added，区分闪避，命中，跳跃，速度，物理防御，魔法防御，手技。
        if (defaultValue == 0) {
            return 0;
        }
        else {//如果锻造的装备属性项不为0，随机属性的算法：10%的概率出现属性直接加+50%，90%的概率出现上限+20%，下限都是-10%
            
            if (Randomizer.nextInt(10) == 1) { //1/10的概率
                return (short) (defaultValue + Math.floor(defaultValue * 0.5));//上限+50%
            } 
            else 
            {
                final int lMaxRange = (int) Math.min(Math.ceil(defaultValue * 0.1), maxRange);//lynfan Math.ceil() 向上取整，即大于这个数的最小的那个整数。
     ...
     private final short getRandStat3() {//lynfan added，区分HP，MP。
        //lynfan 如果锻造的装备其HP和MP属性项为0，随机属性的算法:10%的概率出现500~2000的HP/MP值，剩余90%的概率出现0~500的HP/MP值
            if (Randomizer.nextInt(10) == 1) { //1/10的概率,该方法的作用是生成一个随机的int值，该值介于[0,n)的区间，也就是0到n之间的随机int值，包含0而不包含n
                return (short) (Math.floor(Math.random() * 501)+1500);//lynfan Math.floor() 向下取整, 这里返回的是500~2000的整数。
            } 
            else 
            {
                return (short) (Math.floor(Math.random() * 501));//lynfan Math.floor() 向下取整, 这里返回的是0~500的整数。
            }

    }
    
      private final byte getRandStat4() {//lynfan added，随机装备的可升级次数。10%概率在10~20之间，90%的概率在5~10之间。
    ...
    public final Equip randomizeStats1(final Equip equip,final int equiplevel) {//BOSS掉落装备
        equip.setStr(getRandStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setDex(getRandStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setInt(getRandStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setLuk(getRandStat((short) equiplevel));//lynfan 根据装备等级设置随机属性:10%的概率出现随机属性为10~装备等级之间的值的，90%概率出现随机属性是0~装备等级之间的值
        equip.setMatk(getRandStat1(equip.getMatk(), 20, (short) equiplevel));//lynfan 随机属性的算法：10%的概率出现属性直接加+100%，90%的概率出现上限+20%，下限都是-10%
        equip.setWatk(getRandStat1(equip.getWatk(), 20, (short) equiplevel));//lynfan 随机属性的算法：10%的概率出现属性直接加+100%，90%的概率出现上限+20%，下限都是-10%
        equip.setAcc(getRandStat2(equip.getAcc(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setAvoid(getRandStat2(equip.getAvoid(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setJump(getRandStat2(equip.getJump(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setHands(getRandStat2(equip.getHands(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setSpeed(getRandStat2(equip.getSpeed(), 20));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setWdef(getRandStat2(equip.getWdef(), 50));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setMdef(getRandStat2(equip.getMdef(), 50));//lynfan 随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+10%，下限都是-10%
        equip.setHp(getRandStat3());//lynfan 随机HP属性的算法:10%的概率出现500~1000的HP/MP值，剩余90%的概率出现0~50的HP/MP值
        equip.setMp(getRandStat3());//lynfan 随机MP属性的算法:10%的概率出现500~1000的HP/MP值，剩余90%的概率出现0~50的HP/MP值
        equip.setUpgradeSlots(getRandStat4());//lynfan,随机装备升级次数,10%概率在10~20之间，90%的概率在5~10之间。
        return equip;
    }
  //lynfan BOSS怪物掉落装备属性的代码
  ...
  //lynfan普通怪物掉落装备属性的代码    
    ...
    private static short getRandStat4(short defaultValue, int maxRange) {//lynfan普通怪物掉落装备，随机力量，敏捷，运气和智力随机属性。
        if (defaultValue == 0) {
            return (short) (Math.floor(Math.random() * 11));//lynfan Math.floor() 向下取整, 这里返回的是0~10的整数。
        }
        else {//如果装备属性项不为0，随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+20%，下限都是-20%
            ...
            {
                final int lMaxRange = (int) Math.min(Math.ceil(defaultValue * 0.2), maxRange);//lynfan Math.ceil() 向上取整，即大于这个数的最小的那个整数。
                return (short) ((defaultValue - lMaxRange) + Math.floor(Math.random() * (lMaxRange * 2 + 1)));//这里的2，上限+20%，下限都是-20%。
            }
            
        }
    }
   
    private final short getRandStat5(final short defaultValue, final int maxRange) {//lynfan added，区分物理攻击，魔法攻击，闪避，命中，跳跃，速度，物理防御，魔法防御，手技。
        if (defaultValue == 0) {
            return 0;
        }
        else {//如果锻造的装备属性项不为0，随机属性的算法：10%的概率出现属性直接加+40%，90%的概率出现上限+20%，下限都是-20%
    ...
    private final short getRandStat6() {//lynfan added，区分HP，MP。
        //lynfan 如果装备其HP和MP属性项为0，随机属性的算法:5%的概率出现500~1000的HP/MP值，剩余95%的概率出现0~50的HP/MP值
            if (Randomizer.nextInt(20) == 1) { //1/10的概率,该方法的作用是生成一个随机的int值，该值介于[0,n)的区间，也就是0到n之间的随机int值，包含0而不包含n
                return (short) (Math.floor(Math.random() * 501)+500);//lynfan Math.floor() 向下取整, 这里返回的是500~1000的整数。
            } 
            else 
            {
    ...
    public Equip randomizeStats2(Equip equip) {//lynfan普通怪物掉落装备，随机属性，但是装备的四维属性部分职业是随机给了，有点乱。
        
        equip.setStr(getRandStat4(equip.getStr(), 20));
        equip.setDex(getRandStat4(equip.getDex(), 20));
        equip.setInt(getRandStat4(equip.getInt(), 20));
        equip.setLuk(getRandStat4(equip.getLuk(), 20));
        equip.setMatk(getRandStat5(equip.getMatk(), 20));
        equip.setWatk(getRandStat5(equip.getWatk(), 20));
        equip.setAcc(getRandStat5(equip.getAcc(), 20));
        equip.setAvoid(getRandStat5(equip.getAvoid(), 20));
        equip.setJump(getRandStat5(equip.getJump(), 20));
        equip.setSpeed(getRandStat5(equip.getSpeed(), 20));
        equip.setWdef(getRandStat5(equip.getWdef(), 50));
        equip.setMdef(getRandStat5(equip.getMdef(), 50));
        equip.setHp(getRandStat6());
        equip.setMp(getRandStat6());
        return equip;
    }
    
    
    
    public Equip randomizeStats3(Equip equip,final int equipjob) {//lynfan 判断装备职业给与随机属性，不再盲目的给力量，敏捷，运气，智力了。
        switch (equipjob) {
            case 0://无职业要求
                equip.setStr(getRandStat4(equip.getStr(), 20));
                equip.setDex(getRandStat4(equip.getDex(), 20));
            ...
            case 1://"战士";
                equip.setStr(getRandStat4(equip.getStr(), 20));
                equip.setDex(getRandStat4(equip.getDex(), 20));
                equip.setInt((short)Math.min(equip.getInt(), 100));
                equip.setLuk((short)Math.min(equip.getLuk(), 100));
                break;
            case 2://"法师";
                equip.setStr((short)Math.min(equip.getStr(), 100));
                equip.setDex((short)Math.min(equip.getDex(), 100));
                equip.setInt(getRandStat4(equip.getInt(), 20));
                equip.setLuk(getRandStat4(equip.getLuk(), 20));    
                break;
            case 4://"弓箭手";
