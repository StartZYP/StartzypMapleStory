...
public class MapleMap {
        ...
        for(MonsterDropEntry mde : from) {
            if(!ii.isQuestItem(mde.itemId)) {
                ...
                if(chr.needQuestItem(mde.questid, mde.itemId)) {
                    ...
                    otherQuest.add(mde);
                    // otherQuest.add(mde);//无任务时任务道具不掉落
                }
            }
        }
    }
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) < dropChance) {
                ...
                if (de.itemId == 0) { // meso
                    ...
                    */
                    final int equiplevel=ii.getReqLevel(de.itemId);//lynfan 装备等级，装备随机属性随等级增加
                    final int equipjob = ii.getReqJob(de.itemId);//lynfan 装备职业
                    if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                        
                        if(mob.isBoss()){
                            idrop = ii.randomizeStats1((Equip) ii.getEquipById(de.itemId),equiplevel);//lynfan BOSS 掉落装备随机属性随等级增加
                        }
                        else {
                            //idrop = ii.randomizeStats2((Equip) ii.getEquipById(de.itemId));//lynfan 普通怪物掉落装备随机属性
                            idrop = ii.randomizeStats3((Equip) ii.getEquipById(de.itemId),equipjob);//lynfan 判断装备职业给与随机属性，不在盲目的给力量，敏捷，运气，智力了。
                        }
                        //idrop = ii.randomizeStats((Equip) ii.getEquipById(de.itemId));//lynfan 原始代码
                        } else {
                        idrop = new Item(de.itemId, (short) 0, (short) (de.Maximum != 1 ? Randomizer.nextInt(de.Maximum - de.Minimum) + de.Minimum : 1));
                    }
                    spawnDrop(idrop, calcDropPos(pos, mob.getPosition()), mob, chr, droptype, de.questid);
            ...
            if (Randomizer.nextInt(999999) < de.chance) {
                ...
                if (de.itemId != 0) {
                    ...
                    */
                    ...
                    if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                        //idrop = ii.randomizeStats((Equip) ii.getEquipById(de.itemId));//lynfan 原始代码
                        idrop = ii.randomizeStats1((Equip) ii.getEquipById(de.itemId),equiplevel);//lynfan 装备随机属性随等级增加
                    } else {
                        idrop = new Item(de.itemId, (short) 0, (short) (de.Maximum != 1 ? Randomizer.nextInt(de.Maximum - de.Minimum) + de.Minimum : 1));
                    }
                    spawnDrop(idrop, calcDropPos(pos, mob.getPosition()), mob, chr, droptype, de.questid);
    ...
    */
    ...
    /** 设置所有怪物的重生点 */ //lynfan added below
        private Point rebornPoint;
	private MapleCharacter rebornChr;
	public void setRebornPoint(Point p) {
		rebornPoint = p;
	}
