...
public class MapleShop {
    ...
    public void sell(MapleClient c, MapleInventoryType type, short slot, short quantity) {
        ...
        if(canSell(item, quantity)) {
            ...
            if (!ItemConstants.isScroll(item.getItemId())) {//lynfan added，如果不是卷轴
                if (recvMesos > 0) {
                c.getPlayer().gainMeso(recvMesos, false);
                }
            } else {//lynfan added，如果是卷轴，卖出得绝对音感
                short a = (short)Math.floor((Math.random()*10+5)); //lynfan added，如果是卷轴，卖出得绝对音感
                short b = (short) (a*quantity);
                c.getPlayer().getAbstractPlayerInteraction().gainItem(4310000,b);//lynfan added，如果是卷轴，卖出得绝对音感
            }            
