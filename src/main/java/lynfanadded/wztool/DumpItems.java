...
public class DumpItems {
	...
	private final MapleDataProvider item, character, string = MapleDataProviderFactory.getDataProvider(
			new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/String.wz"));
	...
	public DumpItems(boolean update) throws Exception {
		this.update = update;
		this.item = MapleDataProviderFactory.getDataProvider(
				new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/Item.wz"));
		this.character = MapleDataProviderFactory.getDataProvider(new File(
				(System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/Character.wz"));
	...
	public void dumpItems() throws Exception {
		if (!hadError) {
			...
			PreparedStatement ps = con.prepareStatement(
					"INSERT INTO wz_itemdata(itemid, name, msg, `desc`, slotMax, price, wholePrice, stateChange, flags, karma, meso, monsterBook, itemMakeLevel, questId, scrollReqs, consumeItem, totalprob, incSkill, replaceId, replaceMsg, `create`, afterImage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public void dumpItems(MapleDataProvider d, PreparedStatement psa, PreparedStatement psr, PreparedStatement ps,
			PreparedStatement pse, boolean charz) throws Exception {
		for (MapleDataDirectoryEntry topDir : d.getRoot().getSubdirectories()) { // requirements
																					// first
	...
	public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse,
		...
		if (doneIds.contains(id) || ItemConstants.getInventoryType(id) == MapleInventoryType.UNDEFINED) {
			return;
		}
		doneIds.add(id);
			...
			for (String stat : GameConstants.stats) {
				...
				if (stat.equals("canLevel")) {
					...
					if (stat.equals("skill")) {
						for (int i = 0; i < d.getChildren().size(); i++) { // List
																			// of
																			// allowed
																			// skillIds
	...
	public void dumpItems(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse)
			throws Exception {
		if (!update) {
	...
	protected final MapleData getStringData(final int itemId) {
		...
		} else if ((itemId >= 1132000 && itemId < 1183000) || (itemId >= 1010000 && itemId < 1040000)
