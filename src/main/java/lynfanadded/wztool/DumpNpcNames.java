...
public class DumpNpcNames {
	...
	public void dumpNpcNameData() throws SQLException {
		MapleDataProvider npcData = MapleDataProviderFactory.getDataProvider(
				new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/Npc.wz"));
		MapleDataProvider stringDataWZ = MapleDataProviderFactory.getDataProvider(
		...
		for (MapleData c : npcStringData) {
			...
			try {
				if (npcData.getData(n) != null) {// only thing we really have to
													// do is check if it exists.
													// if we wanted to, we could
													// get the script as well :3
