...
public class DumpOxQuizData {
	...
	public static void main(String args[]) {
		...
		try {
			...
			// FileOutputStream(cashTxt))) {
			// writer.println("INSERT INTO `wz_oxdata` (`questionset`,
			// `questionid`, `question`, `display`, `answer`) VALUES");
	...
	public void dumpOxData() throws SQLException {
		...
		stringProvider = MapleDataProviderFactory.getDataProvider(
				new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/Etc.wz"));
		...
		for (MapleData child1 : ox.getChildren()) {
			for (MapleData child2 : child1.getChildren()) {
				...
				ps.execute();
				ps.close();
				// writer.println("(" + child1.getName() + "," +
				// child2.getName() + ", '" + qs + "', '" + ds + "', '" + as +
				// "'), ");
			}
		}
	}
